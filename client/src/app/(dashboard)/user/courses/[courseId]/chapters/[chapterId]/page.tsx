          ...
          <Tabs defaultValue="Notes" className="course__tabs">
            ...
            <TabsContent className="course__tab-content" value="Resources">
              <Card className="course__tab-card">
                ...
                <CardContent className="course__tab-body">
                  {currentChapter?.pdfResource ? (
                    <a
                      href={currentChapter.pdfResource as string}
                      download={String(currentChapter.pdfResource).split("/").pop()} // Extract filename for download attribute
                      target="_blank" // Open in new tab is safer for downloads
                      rel="noopener noreferrer"
                      className="text-primary-500 hover:underline"
                    >
                      Download PDF Resource: {String(currentChapter.pdfResource).split("/").pop()}
                    </a>
                  ) : (
                    <p className="text-gray-500">No resources available for this chapter.</p>
                  )}
                </CardContent>
              </Card>
            </TabsContent>
